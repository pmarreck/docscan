...
- **`ole2.zig`** (~710 lines) — OLE2 (Compound Binary File) reader. Header parsing (follows the DIFAT chain per MS-CFB §2.5 so files with >109 FAT sectors / >~7MB parse correctly), FAT chain following, directory walking, mini stream support, UTF-16LE→UTF-8 conversion. 11 tests.
- **`endian.zig`** (~14 lines) — Shared little-endian `readU16`/`readU32` helpers used by the OLE2 / ZIP / .doc parsers (previously duplicated byte-for-byte in all three).
...
- **`chunker.zig`** (~660 lines) — Structure-aware chunker. Recursive section walking, breadcrumb paths, paragraph-boundary splitting, small-section merging. Every emitted chunk is force-split to an embed-safe byte budget (`embedSplitBytes` = max_chunk_tokens / tokens_per_byte) so no chunk overflows the embedding model's token limit (was the cause of HTTP 400 "input exceeds maximum tokens" on dense/boundary-less content). 12 tests.
