...
fn jsonSection(out: *std.ArrayList(u8), section: document.Section) !void {
	...
	try out.appendSlice(gpa, ",\"page_physical\":");
	try jsonOptionalU32(out, section.page_physical);
	try out.appendSlice(gpa, ",\"page_logical\":");
	try jsonOptionalU32(out, section.page_logical);
	try out.appendSlice(gpa, ",\"page_section\":");
	try jsonOptionalU32(out, section.page_section);
	try out.appendSlice(gpa, ",\"page_roman\":");
	try out.appendSlice(gpa, if (section.page_roman) "true" else "false");
...
fn jsonChunks(chunks: []const document.Chunk) ![]const u8 {
	...
	for (chunks, 0..) |c, i| {
		...
		try out.appendSlice(gpa, ",\"page_physical\":");
		try jsonOptionalU32(&out, c.page_physical);
		try out.appendSlice(gpa, ",\"page_logical\":");
		try jsonOptionalU32(&out, c.page_logical);
		try out.appendSlice(gpa, ",\"page_section\":");
		try jsonOptionalU32(&out, c.page_section);
		try out.appendSlice(gpa, ",\"page_roman\":");
		try out.appendSlice(gpa, if (c.page_roman) "true" else "false");
		try out.appendSlice(gpa, ",\"source_line\":");
...
fn jsonSearchResults(results: []const document.SearchResult) ![]const u8 {
	...
	for (results, 0..) |r, i| {
		...
		try out.appendSlice(gpa, ",\"page_physical\":");
		try jsonOptionalU32(&out, r.page_physical);
		try out.appendSlice(gpa, ",\"page_logical\":");
		try jsonOptionalU32(&out, r.page_logical);
		try out.appendSlice(gpa, ",\"page_section\":");
		try jsonOptionalU32(&out, r.page_section);
		try out.appendSlice(gpa, ",\"page_roman\":");
		try out.appendSlice(gpa, if (r.page_roman) "true" else "false");
		try out.appendSlice(gpa, ",\"source_line\":");
...
fn jsonChunkRecord(rec: storage.ChunkRecord) ![]const u8 {
	...
	try out.appendSlice(gpa, ",\"page_physical\":");
	try jsonOptionalU32(&out, rec.page_physical);
	try out.appendSlice(gpa, ",\"page_logical\":");
	try jsonOptionalU32(&out, rec.page_logical);
	try out.appendSlice(gpa, ",\"page_section\":");
	try jsonOptionalU32(&out, rec.page_section);
	try out.appendSlice(gpa, ",\"page_roman\":");
	try out.appendSlice(gpa, if (rec.page_roman) "true" else "false");
	try out.appendSlice(gpa, ",\"source_line\":");
