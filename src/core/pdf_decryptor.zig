...
//! ── PROVENANCE ──
//! VENDORED (copied, then lightly adapted) from validate/src/core/pdf_decryptor.zig
//! @ 67f2bb0840c8 on 2026-06-14. This PDF Standard Security Handler logic is SHARED
//! with the `validate` project — if you fix a bug or extend it here, PORT THE FIX
//! BACK to validate (and pull validate's fixes here); neither copy should silently
//! diverge. Pure Zig (std.crypto); no I/O, no C deps — safe in the wasm slice.
...
pub fn parseEncryptionParams(data: []const u8) ?EncryptionParams {
    ...
    // Parse /Length (TOP-LEVEL key length, in bits). Must skip the /CF crypt-filter
    // subdict's own /Length (in BYTES), which for V4-AES precedes the top-level one;
    // reading that as bits clamped to a 40-bit key and produced garbage. Scan at
    // brace-depth 1 only. BUG FIX vs validate/src/core/pdf_decryptor.zig (2026-06-14):
    // port this back to validate (its V4/AES-128 path has the same defect).
    {
        var found_len = false;
        var depth: i32 = 0;
        var p: usize = 0;
